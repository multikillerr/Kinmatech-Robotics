...
from kin_serial import kin_engine, run_f_kin
...
class KinematicWorker(QObject):
    ...
    # Signals
    fk_ready = pyqtSignal(tuple)  # (x, y, z, rx, ry, rz)
    ...
    def _calculate_forward_kinematics(self, joints: List[float]) -> Tuple[float, float, float, float, float, float]:
        """Calculate forward kinematics using real robot model"""
        if len(joints) != 6:
            raise ValueError(f"Expected 6 joint angles, got {len(joints)}")
        
        # Use real FK from kin_serial
        result = run_f_kin(joints[0], joints[1], joints[2], joints[3], joints[4], joints[5])
        
        # run_f_kin returns numpy array [x, y, z, rx, ry, rz]
    ...
    def _calculate_inverse_kinematics(self, pose: Tuple[float, float, float, float, float, float]) -> Optional[List[float]]:
        """Calculate inverse kinematics using real robot model"""
        x, y, z, rx, ry, rz = pose
        
        # Use real IK from kin_serial
        result = kin_engine(x, y, z, rx, ry, rz)
        
        if result is None:
            return None
        
        # kin_engine returns tuple of 6 joint angles
